  ...
  const handleHSSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (hsAuthMode === "signup") {
        ...
        setTimeout(() => {
          setIsHSModalOpen(false);
          setFormData({ firstName: "", lastName: "", age: "", schoolName: "", grade: "", graduationDate: "", email: "", password: "" });
        }, 1500);
      }
    } catch (err: any) {
          ...
          <div className="grid gap-4">
            {[
              ...
              <button 
                key={card.name}
                onClick={() => {
                  if (i === 0) setIsHSModalOpen(true); // Open HS Modal
                  // Undergraduate does nothing (shows tooltip via title)
                }}
                title={i === 1 ? "Coming Soon" : ""} // Tooltip for Undergraduate
              ...
              >
                <div className="flex items-center gap-4">
                  <div className={`w-12 h-12 rounded-xl flex items-center justify-center text-white font-bold text-xl bg-gradient-to-br ${card.color}`}>
                ...
                {/* "Coming Soon" Box for Undergraduate (Visible on Hover) */}
                {i === 1 && (
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
          <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full p-8 relative max-h-[90vh] overflow-y-auto">
            ...
            <p className="text-gray-600 text-center mb-6">
              {hsAuthMode === "login" ? "Welcome back! Sign in to continue." : "Create your profile to get started."}
            </p>
            ...
            <form onSubmit={handleHSSubmit} className="space-y-4">
              ...
              {hsAuthMode === "signup" && (
                <>
                  ...
                  <input
                    ...
                    className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#4CAF50] focus:border-transparent"
                  />
                  <div className="text-sm text-gray-500">Expected Graduation Date</div>
                  <input
                    type="date"
                    required
              ...
              >
                {isLoading ? "Processing..." : (hsAuthMode === "login" ? "Log In" : "Create Account")}
              </button>
            </form>
