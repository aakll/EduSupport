...
export default function Home() {
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col">
      {/* Hero Section */}
      <section className="relative min-h-[85vh] flex items-center justify-center overflow-hidden bg-gradient-to-br from-[#0f172a] via-[#1e293b] to-[#0f172a]">
        
        {/* Animated Background Elements - Subtle Movement */}
        <div className="absolute inset-0 overflow-hidden pointer-events-none">
          <div className={`absolute top-[-10%] left-[-10%] w-96 h-96 bg-blue-500/10 rounded-full blur-3xl transition-all duration-[2000ms] ${mounted ? 'translate-x-20 translate-y-20' : ''}`} />
          <div className={`absolute bottom-[-10%] right-[-10%] w-[500px] h-[500px] bg-indigo-500/10 rounded-full blur-3xl transition-all duration-[2500ms] ${mounted ? '-translate-x-20 -translate-y-20' : ''}`} />
          <div className={`absolute top-[40%] right-[20%] w-64 h-64 bg-emerald-500/5 rounded-full blur-2xl transition-all duration-[3000ms] ${mounted ? 'scale-110' : 'scale-100'}`} />
        </div>

        <div className={`relative z-10 text-center px-4 max-w-3xl mx-auto transition-all duration-1000 ease-out ${mounted ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
          <h1 className="text-5xl md:text-7xl font-extrabold text-white mb-6 tracking-tight drop-shadow-lg">
            EduSupport
          </h1>
          <h2 className="text-2xl md:text-3xl font-medium text-blue-100 mb-4">
            Your Gateway to Academic Success
          </h2>
          <p className="text-lg text-slate-300 mb-12 max-w-xl mx-auto leading-relaxed">
            A comprehensive guide designed specifically for students in Lebanon. Find scholarships, universities, and career paths.
          </p>
          ...
          <div className="grid gap-5 max-w-md mx-auto">
            {cards.map((card, i) => (
              ...
              >
                <div className={`w-12 h-12 rounded-full bg-slate-50 flex items-center justify-center transition-colors duration-300 group-hover:bg-slate-100 ${card.textAccent}`}>
                  {card.icon}
                </div>
                
                <span className="text-lg font-semibold text-slate-800 flex-1">{card.name}</span>
        ...
        {/* Scroll Indicator */}
        <div className={`absolute bottom-8 left-1/2 -translate-x-1/2 transition-opacity duration-1000 delay-1000 ${mounted ? 'opacity-100' : 'opacity-0'}`}>
          <div className="animate-bounce p-2 rounded-full bg-white/10 backdrop-blur-sm">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-fadeIn">
          <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full p-8 relative transform transition-all scale-100">
            ...
            <div className="text-center mb-6">
              ...
              <h3 className="text-2xl font-bold text-gray-900">High School Student</h3>
